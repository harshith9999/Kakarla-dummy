...
import Rows from '../../components/viewBookedRows';
...
const ViewAppointment = ({ uid, role }) => {
    ...
    const fetchData = async () => {
        try {
                setOpenLoader(true)
                const response = await axios.get(`${process.env.REACT_APP_API_URL}/democlass?page=${page}&limit=${rowsPerPage}`)
    ...
    const deleteData = async (id) => {
        setOpenLoader(true)
        try {
            await axios.delete(`${process.env.REACT_APP_API_URL}/democlass/${id}`)
        ...
        openLoader ? <div className='loader'><CircularProgress color="inherit" /></div>
            ...
            <div className='table'>
                {rows?.length ? <div>
                        ...
                        <Table aria-label="collapsible table">
                            <TableHead>
                                <TableRow>
                                    ...
                                    <TableCell sx={{ fontWeight: 'bold' }}>Date of Demo</TableCell>
                    ...
                    <Card>
                        <CardContent>
                            <Typography variant="h5" noWrap component="div" className='apptcard'> No Data </Typography>
                        </CardContent>
                    </Card>)}
